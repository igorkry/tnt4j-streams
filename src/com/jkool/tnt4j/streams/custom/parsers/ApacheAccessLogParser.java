...
/**
 * <p>Implements a custom Apache access log parser based on RegEx parsing. User can define RegEx string to parse log as for ordinary
 * activity RegEx parser.</p>
 * <p>>But it is also possible to use Apache access log configuration pattern over LogPattern parameter. Then RegEx is generated from it.
 * Additional config pattern tokens may be mapped to RegEx'es using ConfRegexMapping parameters.</p>
...
public class ApacheAccessLogParser extends ActivityRegExParser
{
  ...
  /**
   * Defines mapping between Apache access log configuration pattern token strings and RegEx
   * strings used to parse log entries.
   */
  ...
  public void setProperties (Collection<Map.Entry<String, String>> props) throws Throwable
  {
      ...
      else if (PROP_CONF_REGEX_MAPPING.equalsIgnoreCase (name))
      {
        if (!StringUtils.isEmpty (value))
        {
          ...
          if (idx > 0)
          {
            ...
            String oldRegex = configRegexMappings.put (confKey, regex);
            LOGGER.log (OpLevel.DEBUG, "Setting {0} to '{1}'", name, value);
            LOGGER.log (OpLevel.DEBUG, "Setting Apache access log parsing RegEx mapping {0} from '{1}' to '{2}'", confKey, oldRegex, regex);
          }
        }
      }
    ...
    if (pattern == null && StringUtils.isNotEmpty (apacheLogPattern))
    {
      ...
      {
        LOGGER.log (OpLevel.TRACE, "Could not make Apache access log parsing RegEx for '{0}'", apacheLogPattern);
      }
    }
  }
   ...
   *
   * @param apacheLogPattern Apache access log configuration pattern string
   *
   * @return regular expression string, or {@code null} if can't make RegEx string from defined Apache access log configuration
   * pattern string
   */
  private String makeRegexPattern (String apacheLogPattern)
  {
    ...
    //return StringUtils.isEmpty (logRegex) ? null : "(?m)^" + logRegex.trim ();
    return StringUtils.isEmpty (logRegex) ? null : "^" + logRegex.trim ();
  }

  /**
   * Maps Apache access log configuration pattern token to user defined RegEx string. If no user defined mapping is found,
   * then default mapping is used.
   *
   * @param configToken Apache access log configuration pattern token string
   *
  ...
  {
    Set<String> keys = configRegexMappings.keySet ();

    for (String key : keys)
    {
      if (isMatchingPattern (configToken, key))
      {
        return configRegexMappings.get (key);
      }
    }
   ...
   *
   * @param configToken Apache access log configuration token string
   *
   ...
   *
   * @param configToken Apache access log configuration token string
   * @param pattern     pattern string to match configuration token
   *
   * @return true if Apache access log configuration token matches defined pattern
   */
