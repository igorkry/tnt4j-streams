	...
	protected void connectToQmgr() throws Exception {
		...
		if (StringUtils.isEmpty(qmgrName)) {
			LOGGER.log(OpLevel.INFO,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.connecting.default"),
					props);
		} else {
			LOGGER.log(OpLevel.INFO,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.connecting.qm"),
					qmgrName, props);
		}
		...
		if (!StringUtils.isEmpty(topicString) || !StringUtils.isEmpty(topicName) || !StringUtils.isEmpty(subName)) {
			...
			if (!StringUtils.isEmpty(subName)) {
				LOGGER.log(OpLevel.INFO,
						StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ,
			...
			LOGGER.log(OpLevel.INFO,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.opening.queue"),
					qmgrName, String.format("%08X", openOptions)); // NON-NLS
			dest = qmgr.accessQueue(queueName, openOptions);
		}
		LOGGER.log(OpLevel.INFO,
				StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.reading.from"),
	...
	public String getNextItem() throws Exception {
		while (!isHalted() && !isConnectedToQmgr(null)) {
			try {
				...
				if (isConnectedToQmgr(mqe)) {
					...
					LOGGER.log(OpLevel.ERROR, StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ,
				...
				LOGGER.log(OpLevel.INFO,
						StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "TNTInputStream.will.retry"),
		...
		try {
			...
			LOGGER.log(OpLevel.DEBUG,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.waiting.for.message"),
					dest.getName().trim());
			dest.get(mqMsg, gmo);
			LOGGER.log(OpLevel.DEBUG,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.read.msg"),
			...
			LOGGER.log(OpLevel.TRACE,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.message.data"),
			...
			LOGGER.log(OpLevel.ERROR,
					StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.failed.reading"),
					dest.getName().trim(), formatMqException(mqe));
			if (curFailCount >= MAX_CONSECUTIVE_FAILURES) {
				LOGGER.log(OpLevel.ERROR,
						StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ, "WmqStream.reached.limit"),
	...
	protected void closeQmgrConnection() {
		...
		if (qmgr != null) {
			try {
				...
				try {
					LOGGER.log(OpLevel.DEBUG, StreamsResources.getString(WmqStreamConstants.RESOURCE_BUNDLE_WMQ,
